...
void loop() {
  ...
  if (WAITING_FOR_COMMAND) {
    if (RECEIVED_COMMAND) {
      ...
      if (measurementMode == 1) {
        ...
        Serial.println("Test: Stiffness (5 sensors)");
      }
  ...
  // State: Sending Data
    ...
    if (iti_number < max_iti_number) {
      // Prepare data packet with 5 values
      gloveData.index = Array_index[iti_number];
      gloveData.max_index = max_iti_number;
      gloveData.time_ms = Array_time_ms[iti_number];
      gloveData.mode = Array_mode[iti_number];
      gloveData.value1 = Array_value1[iti_number];
      gloveData.value2 = Array_value2[iti_number];
      gloveData.value3 = Array_value3[iti_number];
      gloveData.value4 = Array_value4[iti_number];  // New
      gloveData.value5 = Array_value5[iti_number];  // New
