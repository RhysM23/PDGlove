...
// Data structure for communication - updated to support 5 values
typedef struct struct_message {
    ...
    float value4;    // Additional value (for stiffness mode)
    float value5;    // Additional value (for stiffness mode)
} struct_message;

// Global data object
struct_message gloveData;

// Arrays for storing measurement data - updated for 5 values
...
float Array_value4[MAX_DATA_POINTS];
float Array_value5[MAX_DATA_POINTS];
...
// Store a new data point - updated for 5 values
void storeDataPoint(uint32_t time_ms, int mode, float value1, float value2, float value3, float value4 = 0, float value5 = 0) {
  if (iti_number < MAX_DATA_POINTS) {
    ...
    Array_value4[iti_number] = value4;
    Array_value5[iti_number] = value5;
